...
class HarpoonSpec(object):
    ...
    def tasks_spec(self, available_actions, default_action="run"):
        ...
        return sb.dictof(
            ...
            sb.create_spec(
                ...
                action=sb.defaulted(
                    sb.string_choice_spec(available_actions, "No such task"), default_action
                ),
    ...
    def context_spec(self):
        ...
        return sb.dict_from_bool_spec(
            ...
            sb.create_spec(
                ...
                parent_dir=sb.directory_spec(
                    sb.formatted(
                        sb.defaulted(sb.string_spec(), "{config_root}"),
    ...
    def image_spec(self):
        ...
        return sb.create_spec(
            ...
            ,
            tag=sb.optional_spec(
                sb.formatted(sb.string_spec(), formatter=MergedOptionStringFormatter)
            ),
            name=sb.formatted(
                sb.defaulted(sb.string_spec(), "{_key_name_1}"),
                formatter=MergedOptionStringFormatter,
            ),
            key_name=sb.formatted(
                sb.overridden("{_key_name_1}"), formatter=MergedOptionStringFormatter
            ),
            image_name=sb.optional_spec(sb.string_spec()),
            image_index=sb.formatted(
                sb.defaulted(sb.string_spec(), ""), formatter=MergedOptionStringFormatter
            ),
            container_name=sb.optional_spec(sb.string_spec()),
            image_name_prefix=sb.defaulted(sb.string_spec(), ""),
            no_tty_option=sb.defaulted(
                sb.formatted(sb.boolean(), formatter=MergedOptionStringFormatter), False
            ),
            user=sb.defaulted(sb.string_spec(), None),
            configuration=sb.any_spec(),
            vars=sb.dictionary_spec(),
            assume_role=sb.optional_spec(
                sb.formatted(sb.string_spec(), formatter=MergedOptionStringFormatter)
            ),
            ...
            ,
            shell=sb.defaulted(
                sb.formatted(sb.string_spec(), formatter=MergedOptionStringFormatter), "/bin/bash"
            ),
            bash=sb.delayed(
                sb.optional_spec(
                    sb.formatted(sb.string_spec(), formatter=MergedOptionStringFormatter)
