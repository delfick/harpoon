...
describe HarpoonCase, "GitMtimes":
    ...
    describe "find_for":
        ...
        def find_for(self, context, silent_build):
            return GitMtimes(context.git_root, context.parent_dir, context.use_git_timestamps, context.include, context.exclude, silent_build).find_for()
        ...
        it "is able to find all the files owned by git and get their last commit modified time":
            with self.cloned_repo_example() as root_folder:
                ...
                self.assertEqual(self.find_for(ctxt, False), self.repo_example_map())
        ...
        it "only includes files under the parent_dir":
            with self.cloned_repo_example() as root_folder:
                ...
                self.assertEqual(self.find_for(ctxt, False), expected_map)
